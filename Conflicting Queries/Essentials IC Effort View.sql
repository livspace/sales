--http://reports.livspace.com/question/2431
...
count(distinct(case when EXTRACT(MONTH FROM prs2.probable_month)=10 and prs2.conversion_probability='high' and prs.project_stage_weight<400 then prs.project_id end)) as high,
count(distinct(case when EXTRACT(MONTH FROM prs2.probable_month)=10 and prs2.conversion_probability='medium' and prs.project_stage_weight<400 then prs.project_id end)) as med,

count(distinct(case when EXTRACT(MONTH FROM prs2.probable_month)=10 and prs2.conversion_probability='high' and prs.project_stage_weight<400 then prs.project_id end))*0.8
+
count(distinct(case when EXTRACT(MONTH FROM prs2.probable_month)=10 and prs2.conversion_probability='medium' and prs.project_stage_weight<400 then prs.project_id end))*0.5
as expected,
