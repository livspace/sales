--http://reports.livspace.com/question/1865
...
Coalesce(sum(case when B.month = extract(month from current_date+0) and B.year = extract(year from current_date+0) then (B.gmv)/100000 end),0) as M0,
  /*coalesce(sum((B.gmv)/100000),0) as Total,*/
  Coalesce(sum(case when B.month = extract(month from current_date) and B.year = 2019 then ((B.gmv)/(extract(day from (current_date))-1))*30 end)/100000,0) as M0_Projection
  from
  (
        Select A.*
        from
...
152616, 330586, 209475, 272877, 205554, 366074, 373736, 391971, 391975, 244482, 382074, 382047, 400526, 394450, 13212,  330301, 0,  95945,  44496,  4492, 189999 ) /*-- test projects, DVs, liquidation, Raw_material buyback, One Commercial */
          ...
          and to_date(fo.order_created_at,'YYYY-MM-DD') >=date_trunc('month', current_date - interval '6 month')
...
Select   CGMV.Managed_city as BM,
   ...
   -- (CGMV.Feb) Feb_2019,
       ...
       to_char((CGMV.M0_6),'FM9,999D0') M0_6,
       to_char((CGMV.M0_5),'FM9,999D0') M0_5,
       to_char((CGMV.M0_4),'FM9,999D0') M0_4,
       to_char((CGMV.M0_3),'FM9,999D0') M0_3,
       to_char((CGMV.M0_2),'FM9,999D0') M0_2,
       to_char((CGMV.M0_1),'FM9,999D0') M0_1,
       to_char((CGMV.M0),'FM9,999D0') M0,
       /*to_char((CGMV.Total),'FM9,9999D0') Total,*/
       to_char((CGMV.M0_projection),'FM9,999D0') M0_2019_Projection,
       concat(to_char((((CGMV.M0_projection - CGMV.M0_1)/NULLIF(CGMV.M0_1,0))*100),'FM9,99D0'),'%')as MOM
 ...
 other_1 as (
   ...
   to_char(sum((CGMV.M0_6)),'FM9,999D0') M0_6_2019,
   to_char(sum((CGMV.M0_5)),'FM9,999D0') M0_5_2019,
   to_char(sum((CGMV.M0_4)),'FM9,999D0') M0_4_2019,
   to_char(sum((CGMV.M0_3)),'FM9,999D0') M0_3_2019,
   to_char(sum((CGMV.M0_2)),'FM9,999D0') M0_2_2019,
   to_char(sum((CGMV.M0_1)),'FM9,999D0') M0_1_2019,
   to_char(sum((CGMV.M0)),'FM9,999D0') M0_2019,
  /* to_char(sum((CGMV.Total)),'FM9,9999D0') Total,*/
   to_char(sum((CGMV.M0_projection)),'FM9,999D0') M0_2019_Projection,
   concat(to_char(((sum(CGMV.M0_projection - CGMV.M0_1)/sum(CGMV.M0_1))*100),'FM9,99D0'),'%')as MOM
