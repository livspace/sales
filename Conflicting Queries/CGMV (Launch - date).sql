--http://reports.livspace.com/question/2338
...
sum(case when lower(C.super_category) like '%knw%' then (coalesce(C.Total_price_wt,0) + coalesce(Handling_fees))end) as knw,
sum(case when lower(C.super_category) like '%furniture%' then (coalesce(C.Total_price_wt,0) + coalesce(Handling_fees))end) as furniture,
sum(case when lower(C.super_category) like '%decor%' then (coalesce(C.Total_price_wt,0) + coalesce(Handling_fees))end) as decor,
sum(case when lower(C.super_category) like '%services%' then (coalesce(C.Total_price_wt,0) + coalesce(Handling_fees))end) as services,
sum(C.GMV) as GMV
...
left join cms_backend.cms_super_category as smpc on pt.super_category = smpc.id
 ...
 and p.is_test = 0 and lower(p1.client) !='oms'
...
case when oo.Project_id = 263836 then 'Bangalore'
 ...
 select project_id, primary_designer_id , bu.name
	...
	-- and to_date(oo.created_at,'YYYY-MM-DD') >= '2018-04-01'
	 ...
	 left join flat_tables.flat_project_events pvs on C.project_id=pvs.project_id and pvs.new_value=5  
...
and C.Primary_CM in ('Nidhi Agarwal','Anjuli Gupta','Abhishek Sharma','Palak Jolly','Rinkesh Bafna','Gautam Gupta','Sri Harsha Ks','Mohammed Azharuddin','Tarun Garg','Prateek Bharadwaj','Himanshi Sharma','Abhishek Sharma','Heli Kumar','Chandana  Ainwale','Annu  Bala','Natasha Sharon Shah','Natasha .','Satish Prasad  Sahu','Mohammad Takki','Shuchi',
'Nitin Bhatia') 

and pvs.event_date>='2019-09-15'::date
