--http://reports.livspace.com/question/2534
...
/
nullif((count(distinct(case when prs.stage_id in (23,13) and prs.project_status = 'ACTIVE' then prs.project_id end)) )::numeric,0)
as bsc_pct,
...
then prs.project_id end))*100)
/
nullif((count(distinct(case when prs.stage_id in (9,19,20) and prs.project_status = 'ACTIVE' then prs.project_id end)) )::numeric,0)
as bd_pct
