--http://reports.livspace.com/question/1997
...
(count(distinct(case when prs.inactivation_reason like '%change_of_plan%' then prs.project_id end))*100)/count(distinct prs.project_id) as "change of plan %"
...
where 
(lower(prs.primary_cm) LIKE '%chandani%' or lower(prs.primary_cm) LIKE '%azharuddin%' or lower(prs.primary_cm) LIKE '%abhishek sharma%' or lower(prs.primary_cm) LIKE '%naufil%' or lower(prs.primary_cm) LIKE '%natasha%' or lower(prs.primary_cm) LIKE '%jolly%' or lower(prs.primary_cm) LIKE '%vidyuth%' or lower(prs.primary_cm) LIKE '%nidhi agarwal%' or lower(prs.primary_cm) LIKE '%aditi mukherjee%') 
and prs.project_stage_weight>=250 and prs.project_status='INACTIVE' and (pvs.created_at>= date_trunc('month', current_date))
