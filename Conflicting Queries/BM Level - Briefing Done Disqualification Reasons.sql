--http://reports.livspace.com/question/1857
...
(sum(case when prs.inactivation_reason = 'client_doesnot_want_to_work_with_us' then 1.0 else 0 end)/nullif(sum(case when prs.status = 'INACTIVE' then 1.0 else 0 end),0)*100) 
as client_does_not_want_Percent,

(sum(case when prs.inactivation_reason = 'change_of_plan' then 1.0 else 0 end)/nullif(sum(case when prs.status = 'INACTIVE' then 1.0 else 0 end),0)*100) as change_of_plan_Percent
