--http://reports.livspace.com/question/1842
...
(
  ...
  coalesce(sum(case when B.month = 6 and B.year = 2019 then ((B.gmv)/(extract(day from (current_date))-1))*30 end)/100000,0) as Jun_Projection
  from
  (
        Select A.*
        from
...
152616, 330586, 209475, 272877, 205554, 366074, 373736, 391971, 391975, 244482, 382074, 382047, 400526, 394450, 13212,  330301, 0,  95945,  44496,  4492, 189999 ) /*-- test projects, DVs, liquidation, Raw_material buyback, One Commercial */
          ...
          and bu1.name in ('Debica Potsangbam', 'Kalyani A', 'Monica Reddy', 'Sneha Joshi', 'Suresh Nagala','Monica K', 'Nilisha Keshri', 'Sohail Ahmed', 'Mir Iqbal Khan', 'Hitechchha Deva')
          ) as A
...
Select   CGMV.Managed_city as BM,
   ...
   -- (CGMV.Feb) Feb_2019,
     ...
     to_char((CGMV.Apr),'FM9,999D0') Apr_2019,
       ...
       to_char((CGMV.Jun_projection),'FM9,999D0') Jun_2019_Projection,
       concat(to_char((((CGMV.Jun_projection - CGMV.May)/NULLIF(CGMV.May,0))*100),'FM9,99D0'),'%')as MOM
 ...
 other_1 as (
  ...
  /* to_char(sum((CGMV.Total)),'FM9,9999D0') Total,*/
   to_char(sum((CGMV.Jun_projection)),'FM9,999D0') Jun_2019_Projection,
   concat(to_char(((sum(CGMV.Jun_projection - CGMV.May)/sum(CGMV.May))*100),'FM9,99D0'),'%')as MOM
