--http://reports.livspace.com/question/2254
...
count(distinct(case when pvs.new_value=23 then prs.project_id end)) as Total_BCS_Count,
--(nullif(round(count(distinct(case when (pvs.new_value=23 and pvs.event_date< CURRENT_DATE - INTERVAL '7 days') then prs.project_id end)),0),2)/nullif(count(distinct(case when pvs.new_value=23 then prs.project_id end)),0)*100) as BCS_Decay_Percent,
...
count(distinct(case when pvs.new_value=9 then prs.project_id end)) as Total_BD_Count
--(nullif(round(count(distinct(case when (pvs.new_value=9 and pvs.event_date< CURRENT_DATE - INTERVAL '7 days') then prs.project_id end)),0),2)/nullif(count(distinct(case when pvs.new_value=9 then prs.project_id end)),0)*100) as BD_Decay_Percent
