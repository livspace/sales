...
((month(prs.created_at) = month(current_date())-1 and year(prs.created_at)=year(current_date()))
or
(month(prs.created_at) = month(current_date())+11 and year(prs.created_at)=year(current_date())-1))

 and ct.name like "%bang%" and cm.name is not null
